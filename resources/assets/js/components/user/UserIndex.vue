  ...
  <v-container>
    <v-toolbar>
      ...
      <v-dialog
        ...
        v-model="dialog"
        width="500"
        @keydown.esc="$router.go({name:'userIndex', params: this.$route.params})"
        :fullscreen="loading"
      >
        <v-btn
          slot="activator"
        ...
        <v-card :class="loading ? 'transparent' : ''">
          ...
          <div v-else>
            ...
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn color="success" small @click.native="$router.go({name:'userIndex', params: this.$route.params})"><v-icon small>check</v-icon> Cerrar</v-btn>
            </v-card-actions>
          </div>
        </v-card>
      </v-dialog>
