              ...
              <v-list-tile-content>
                <v-layout wrap row>
                  <v-flex>
                    ...
                    <v-list-tile-sub-title v-text="`${item.consultant_position.name}`"></v-list-tile-sub-title>
                  </v-flex>
                </v-layout>
              </v-list-tile-content>
              <v-divider
          ...
          <v-container v-if="this.contract.start_date">
            <v-list>
              ...
              <v-list-tile>
                ...
                <v-list-tile-content class="align-end">{{ this.contract.consultant_position.name }}</v-list-tile-content>
              </v-list-tile>
              <v-list-tile>
                <v-list-tile-content class="font-weight-bold">Unidad:</v-list-tile-content>
                <v-list-tile-content class="align-end">{{ this.contract.consultant_position.position_group.name }}</v-list-tile-content>
              </v-list-tile>
            </v-list>
          </v-container>
        </v-flex>
    ...
    async addPayroll() {
      try {
        if (this.contract.employee.identity_card) {
          let res = await axios.post(`/consultant_payroll`, {
            consultant_procedure_id: this.procedure.id,
            consultant_contract_id: this.contract.id,
            employee_id: this.contract.employee.id,
            charge_id: this.contract.consultant_position.charge.id,
            position_group_id: this.contract.consultant_position.position_group.id,
            consultant_position_id: this.contract.consultant_position.id
          });
      ...
      try {
        if (!value) {
          ...
          let res = await axios.get(
            `/consultant_payroll/procedure/${this.procedure.id}`,
            {
              params: {
                consultant_contract_id: this.contract.id,
                employee_id: this.contract.employee.id,
