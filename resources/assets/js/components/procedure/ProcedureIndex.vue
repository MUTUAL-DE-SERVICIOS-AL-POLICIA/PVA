    ...
    <v-card v-else>
      ...
      >
        <v-layout row wrap>
          <v-flex
            md6 lg4
            v-if="bonusYear.bonus > 0 && ($store.getters.permissions.includes('create_procedure') || $store.getters.role == 'admin')"
          >
            ...
            <v-card :color="procedure.active ? 'warning' : 'green lighten-4'" height="100%">
              ...
              <div>
                <v-card-actions v-if="!procedure.new">
                  <v-spacer></v-spacer>
                  <v-btn icon v-if="(procedure.active && $store.getters.permissions.includes('update_procedure')) || $store.getters.role == 'admin'" :to="{ name: 'procedureEdit', params: { id: procedure.id }}" >
                    <v-tooltip top>
                  ...
                  <v-menu offset-y v-if="$store.getters.role == 'rrhh' || $store.getters.role == 'admin'">
                    <v-btn slot="activator" :color="procedure.active ? 'info' : 'primary'">
                      <span>Planillas</span>
                      <v-icon small>arrow_drop_down</v-icon>
                    </v-btn>
                ...
                <v-card-actions v-else>
                  ...
                  <v-btn
                    ...
                    v-if="$store.getters.permissions.includes('create_procedure') || $store.getters.role == 'admin'"
                  >
                    Registrar
                  </v-btn>
                </v-card-actions>
