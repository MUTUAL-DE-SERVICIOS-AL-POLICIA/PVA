...
class EmployeeDeparture
{
  ...
  {
    $start_time = Carbon::parse($departure->contract->job_schedules->start_hour . ':' . $departure->contract->job_schedules->start_minutes)->format('H:i:s');
    $end_time = Carbon::parse($departure->contract->job_schedules->end_hour . ':' . $departure->contract->job_schedules->end_minutes)->format('H:i:s');
    if ($diffdays == 0) {
